...
const STATUS_MAP: Record<string, { label: string; color: string; icon: React.ReactNode }> = {
    ...
    APPROUVEE:  { label: "Approuvée",   color: "text-emerald-400 bg-emerald-500/10 border-emerald-500/20", icon: <CheckCircle className="w-3 h-3" /> },
    REJETEE:    { label: "Rejetée",     color: "text-red-400 bg-red-500/10 border-red-500/20",           icon: <XCircle className="w-3 h-3" /> },
};
...
export default function AdminAdvanceRequestsPage() {
    ...
    const fetchData = useCallback(async () => {
        const [aRes, pRes] = await Promise.all([fetch("/api/advance-requests"), fetch("/api/projects")]);
        if (aRes.ok) setRequests(await aRes.json());
    ...
    return (
        <div className="space-y-6">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 animate-fade-in">
                <div>
                    <p className="text-xs font-semibold uppercase tracking-widest text-primary/80 mb-1">Espace Administrateur</p>
                    <h1 className="text-2xl lg:text-3xl font-bold tracking-tight flex items-center gap-3">
                        <CircleDollarSign className="w-8 h-8 text-primary" /> Demandes de Fonds
                    </h1>
                    <p className="text-sm text-slate-400 mt-1">Gérez les avances sur salaire et besoins en fonds</p>
                </div>
            ...
            <div className="grid grid-cols-3 gap-3 animate-fade-in stagger-1">
                {Object.entries(STATUS_MAP).map(([k, v]) => (
                    <div key={k} className="card !py-3 text-center">
                        <p className={`text-xl font-bold ${v.color.split(" ")[0]}`}>{requests.filter(r => r.status === k).length}</p>
                                ...
                                <div className="flex items-start gap-4 flex-1 min-w-0">
                                    ...
                                    <div className="min-w-0">
                                        <div className="flex items-baseline gap-3 mb-1">
                                            ...
                                            <span className="text-xs text-slate-500">{r.user.firstName} {r.user.lastName}</span>
                                        </div>
                                        <p className="text-sm text-slate-300 mb-1">{r.reason}</p>
                                        <p className="text-xs text-primary font-medium">{r.project.name} — {fmtDate(r.createdAt)}</p>
                                    </div>
                                </div>
                                <div className="flex items-center gap-3 shrink-0">
                                    <span className={`badge border text-xs flex items-center gap-1.5 ${st.color}`}>{st.icon} {st.label}</span>
                                    {r.status === "EN_ATTENTE" && (
                                        <div className="flex gap-2">
                                            <button title="Approuver" onClick={() => handleStatusUpdate(r.id, "APPROUVEE")} className="p-2 rounded-xl bg-emerald-500/10 text-emerald-400 hover:bg-emerald-500/20 transition-colors">
                                                <CheckCircle className="w-4 h-4" />
                                            </button>
                                            <button title="Rejeter" onClick={() => handleStatusUpdate(r.id, "REJETEE")} className="p-2 rounded-xl bg-red-500/10 text-red-400 hover:bg-red-500/20 transition-colors">
                ...
                <div className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-4" onClick={() => setShowModal(false)}>
                    <div className="bg-[#0d1626] border border-white/10 rounded-[2rem] w-full max-w-lg p-6" onClick={e => e.stopPropagation()}>
                        ...
                        <div className="space-y-4">
                            <div>
                                <label className="block text-[10px] uppercase tracking-widest font-bold text-slate-500 mb-2">Projet concerné *</label>
                            ...
                            <div>
                                <label className="block text-[10px] uppercase tracking-widest font-bold text-slate-500 mb-2">Motif de la demande *</label>
