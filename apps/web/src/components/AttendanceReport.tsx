...
import { Project, Team } from "@lynx/types";
import { useCheckIn, useCheckOut, useProjects } from "@lynx/api-client";
...
export default function AttendanceReport() {
    ...
    const [location, setLocation] = useState<string>("Web Pointage");
    const { mutate: checkIn, isPending: checkInPending } = useCheckIn();
    const { mutate: checkOut, isPending: checkOutPending } = useCheckOut();

    const handleCheckIn = () => {
        if (!navigator.geolocation) {
            checkIn({ location: "GPS Non supporté (Web)" }, { onSuccess: fetchData });
            return;
        }
        navigator.geolocation.getCurrentPosition(
            (pos) => checkIn({ location: `${pos.coords.latitude}, ${pos.coords.longitude}` }, { onSuccess: fetchData }),
            () => checkIn({ location: "Localisation refusée (Web)" }, { onSuccess: fetchData })
        );
    };

    const handleCheckOut = () => {
        if (!navigator.geolocation) {
            checkOut({ location: "GPS Non supporté (Web)" }, { onSuccess: fetchData });
            return;
        }
        navigator.geolocation.getCurrentPosition(
            (pos) => checkOut({ location: `${pos.coords.latitude}, ${pos.coords.longitude}` }, { onSuccess: fetchData }),
            () => checkOut({ location: "Localisation refusée (Web)" }, { onSuccess: fetchData })
        );
    };
    ...
    return (
        <div className="space-y-6">
            {/* Self-Service Web Pointage */}
            <div className="card border-primary/20 shadow-lg shadow-primary/5 p-6 animate-fade-in">
                <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
                    <div>
                        <h2 className="text-xl font-bold flex items-center gap-2">
                            <Clock className="w-5 h-5 text-primary" /> Mon Pointage Rapide (Web)
                        </h2>
                        <p className="text-sm text-slate-400 mt-1">Enregistrez votre arrivée ou départ directement depuis le tableau de bord.</p>
                    </div>
                    <div className="flex gap-3">
                        <button 
                            onClick={handleCheckIn}
                            disabled={checkInPending}
                            className="bg-emerald-500 hover:bg-emerald-600 text-white font-bold py-3 px-6 rounded-2xl flex items-center gap-2 transition-all shadow-lg shadow-emerald-500/20 disabled:opacity-50"
                        >
                            {checkInPending ? <Loader2 className="w-4 h-4 animate-spin" /> : <Clock className="w-4 h-4" />}
                            Arrivée
                        </button>
                        <button 
                            onClick={handleCheckOut}
                            disabled={checkOutPending}
                            className="bg-blue-500 hover:bg-blue-600 text-white font-bold py-3 px-6 rounded-2xl flex items-center gap-2 transition-all shadow-lg shadow-blue-500/20 disabled:opacity-50"
                        >
                            {checkOutPending ? <Loader2 className="w-4 h-4 animate-spin" /> : <LogOut className="w-4 h-4" />}
                            Départ
                        </button>
                    </div>
                </div>
