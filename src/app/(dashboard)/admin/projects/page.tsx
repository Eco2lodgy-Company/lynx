    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 animate-fade-in">
                <div>
                    ...
                    <p className="text-sm text-slate-400 mt-1">
                        {stats.total} projet{stats.total !== 1 ? "s" : ""} — {stats.enCours} en cours
                    </p>
                </div>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 animate-fade-in stagger-1">
                {[
                    { label: "Total", value: stats.total, icon: <FolderKanban className="w-5 h-5" />, color: "text-primary" },
                    { label: "En cours", value: stats.enCours, icon: <TrendingUp className="w-5 h-5" />, color: "text-amber-400" },
                    { label: "Planifiés", value: stats.planifie, icon: <Clock className="w-5 h-5" />, color: "text-blue-400" },
                    { label: "Terminés", value: stats.termine, icon: <CheckCircle2 className="w-5 h-5" />, color: "text-emerald-400" },
                ].map((stat) => (
                    <div key={stat.label} className="card flex items-center gap-4">
                        <div className={`${stat.color} p-2 rounded-xl bg-current/10`}>{stat.icon}</div>
                        <div>
            ...
            {loading ? (
                ...
                <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3 animate-fade-in stagger-3">
                        ...
                        return (
                            <div key={project.id} className="card hover:border-primary/30 transition-all group">
                                <div className="flex items-start justify-between mb-3">
                                    ...
                                    <div className="flex items-center gap-1 ml-2 shrink-0">
                                        <button
                                            ...
                                            className="p-1.5 text-slate-500 hover:text-primary rounded-lg transition-colors opacity-0 group-hover:opacity-100"
                                            title="Modifier"
                                        >
                                ...
                                <div className="flex flex-wrap gap-2 mb-3">
                                    ...
                                    <span className={`badge border-transparent text-xs ${pr.color} bg-current/10`}>
                                        {pr.label}
                                    </span>
                                </div>
                                ...
                                <div className="grid grid-cols-2 gap-2 text-xs text-slate-400">
                                    ...
                                    <div className="flex items-center gap-1">
                                        ...
                                        {project._count.incidents} incident{project._count.incidents !== 1 ? "s" : ""}
                                    </div>
                                    <div className="flex items-center gap-1">
            ...
            {showModal && (
                <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4" onClick={() => setShowModal(false)}>
                    <div
                        className="bg-surface-dark border border-border-dark rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto animate-fade-in"
                    ...
                    >
                        ...
                        <form onSubmit={handleSubmit} className="p-6 space-y-4">
                            ...
                            <div className="space-y-1">
                                <label className="text-xs font-medium text-slate-400">Nom du projet *</label>
                                <input
                                    type="text"
                                    value={form.name}
                                    ...
                                    required
                                    placeholder="Ex: Villa Moderne Hydra"
                                />
                            </div>
                            ...
                            <div className="space-y-1">
                                <label className="text-xs font-medium text-slate-400">Adresse du chantier</label>
                                <input
                                    type="text"
                                    value={form.address}
                                    onChange={(e) => setForm({ ...form, address: e.target.value })}
                                    className="input-field"
                                    placeholder="Ex: Lot 42, Cité Hydra, Alger"
                                />
                            </div>
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div className="space-y-1">
                                    <label className="text-xs font-medium text-slate-400">Date de début</label>
                                    <input
                                        type="date"
                                ...
                                <div className="space-y-1">
                                    <label className="text-xs font-medium text-slate-400">Date de fin estimée</label>
                                    <input
                                        type="date"
                            ...
                            <div className="space-y-1">
                                <label className="text-xs font-medium text-slate-400">Budget du projet (€)</label>
                                <input
                                    type="number"
                                    value={form.budget}
                            ...
                            <div className="space-y-1">
                                <label className="text-xs font-medium text-slate-400">Département responsable</label>
                                <div className="relative">
                                    <select
