...
const STATUS_MAP: Record<string, { label: string; color: string }> = {
    ...
    EN_TRANSIT:  { label: "En transit",  color: "text-blue-400 bg-blue-500/10 border-blue-500/20" },
    LIVREE:      { label: "Livré",        color: "text-emerald-400 bg-emerald-500/10 border-emerald-500/20" },
    RETARDEE:    { label: "Retardé",      color: "text-red-400 bg-red-500/10 border-red-500/20" },
};
...
export default function AdminDeliveriesPage() {
    ...
    const fetchData = useCallback(async () => {
        const [dRes, pRes] = await Promise.all([fetch("/api/deliveries"), fetch("/api/projects")]);
                        ...
                        return (
                            <div key={d.id} className="card flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                                ...
                                <div className="flex items-center gap-3 shrink-0">
                                    ...
                                    {d.status === "LIVREE" && <CheckCircle className="w-5 h-5 text-emerald-400" />}
                                </div>
                            </div>
                        );
                    })}
                        ...
                        <div className="space-y-4">
                            <div>
                                ...
                                <div className="relative">
                                    <select value={form.projectId} onChange={e => setForm({...form, projectId: e.target.value})} className="input-field w-full pr-10 appearance-none" title="Sélectionner un projet">
